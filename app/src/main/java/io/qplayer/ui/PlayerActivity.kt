...
class PlayerActivity : AppCompatActivity(), View.OnClickListener {
	...
	private val ivHideControllerButton: ImageView by bindView(R.id.exo_controller)
	private val ivSettings: ImageView by bindView(R.id.settings)
	...
	private var uriString: String = "http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4"
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		//Check the existing state of the system to know whether or not
		...
		if (intent.hasExtra(EXTRA_VIDEO_URI)) {
			uriString = intent.getStringExtra(EXTRA_VIDEO_URI)
		} else if (intentReader.isShareIntent) {
			//Get Uri from stream
			uriString = intentReader.stream.toString()
		}
		println(uriString)
		
		shouldAutoPlay = true
		@Suppress("UNCHECKED_CAST")
		mediaDataSourceFactory = DefaultDataSourceFactory(this, Util.getUserAgent(this, getString(R.string.application_name)),
			...
			if (trackGroups !== lastSeenTrackGroupArray) {
				...
				if (mappedTrackInfo != null) {
					if (mappedTrackInfo.getTypeSupport(C.TRACK_TYPE_VIDEO) == MappingTrackSelector.MappedTrackInfo.RENDERER_SUPPORT_UNSUPPORTED_TRACKS) {
						Toast.makeText(this@PlayerActivity, "Error unsupported track", Toast.LENGTH_SHORT).show()
					}
				}
